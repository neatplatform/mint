// Package grace coordinates client connections, server startup, retries, and graceful shutdown.
package grace
...
// Client represents a dependency that connects on startup and disconnects during shutdown.
...
// Server represents a service that starts serving and shuts down gracefully.
...
// RegisterClient registers clients to connect on startup and disconnect during shutdown.
func RegisterClient(c ...Client) {
	clients = append(clients, c...)
}

// RegisterServer registers servers to start and shut down gracefully.
func RegisterServer(s ...Server) {
	servers = append(servers, s...)
}

// SetMaxRetry sets the maximum number of client connection retries.
// The default is 5.
func SetMaxRetry(n int) {
	maxRetry = n
}

// SetGracePeriod sets how long shutdown can take for servers and clients.
// If the grace period expires, the process terminates ungracefully.
...
func connectWithRetry(c Client, retries int) error {
	...
	logger.Infof("Retrying connecting client: %s", c)
...
// StartAndWait connects all registered clients, starts all registered servers, and
// blocks until shutdown is triggered.
//
// A graceful shutdown starts when a client cannot connect after retries, a server
// returns an error, or the process receives SIGHUP, SIGINT, SIGQUIT, or SIGTERM.
// If a second shutdown signal arrives before shutdown completes, the process exits
