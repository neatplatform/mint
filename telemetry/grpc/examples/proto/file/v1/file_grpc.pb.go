...
const (
	...
	FileService_Sync_FullMethodName        = "/file.v1.FileService/Sync"
)
...
func (c *fileServiceClient) Sync(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[SyncRequest, SyncResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &FileService_ServiceDesc.Streams[2], FileService_Sync_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FileService_SyncClient = grpc.BidiStreamingClient[SyncRequest, SyncResponse]
...
func (UnimplementedFileServiceServer) Sync(grpc.BidiStreamingServer[SyncRequest, SyncResponse]) error {
...
func _FileService_Sync_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(FileServiceServer).Sync(&grpc.GenericServerStream[SyncRequest, SyncResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FileService_SyncServer = grpc.BidiStreamingServer[SyncRequest, SyncResponse]
