...
func main() {
	...
	p := telemetry.NewProbe(
		...
		telemetry.WithForwardLogger(
			"info", "logs.echo-service",
			"localhost:24224", false, nil,
		),
	)
	...
	// Give the Forward client a moment to batch and push the logs before the process exits.
